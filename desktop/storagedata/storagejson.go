...
func decLoadedData(dto PassDto) (*LoadedItems, error) {
	...
	if common.IsWhiteAndSpace(loadedItems.Preferences.LockPwd) {
		// 若启动密码不是空的，但却存在密码项！说明有人篡改了本地数据文件！直接错误返回给前端
	...
	for i := 0; i < len(loadedItems.Data); i++ {
		loadedItems.Data[i].Name, err = common.DecryptAES([]byte(loadedItems.Preferences.LockPwd), loadedItems.Data[i].Name)
		loadedItems.Data[i].Site, err = common.DecryptAES([]byte(loadedItems.Preferences.LockPwd), loadedItems.Data[i].Site)
		loadedItems.Data[i].Remark, err = common.DecryptAES([]byte(loadedItems.Preferences.LockPwd), loadedItems.Data[i].Remark)
		loadedItems.Data[i].Password, err = common.DecryptAES([]byte(loadedItems.Preferences.LockPwd), loadedItems.Data[i].Password)
		loadedItems.Data[i].AccountName, err = common.DecryptAES([]byte(loadedItems.Preferences.LockPwd), loadedItems.Data[i].AccountName)
	}
	if err != nil {
...
func EncryLoadedData(data LoadedItems) error {
	...
	s := ""
	// 先用前端传递过来的明文启动密码加密所有密码项
	for i := 0; i < len(data.Data); i++ {
		data.Data[i].Name, err = common.EncryptAES([]byte(data.Preferences.LockPwd), data.Data[i].Name)
		data.Data[i].Site, err = common.EncryptAES([]byte(data.Preferences.LockPwd), data.Data[i].Site)
		data.Data[i].Remark, err = common.EncryptAES([]byte(data.Preferences.LockPwd), data.Data[i].Remark)
		data.Data[i].Password, err = common.EncryptAES([]byte(data.Preferences.LockPwd), data.Data[i].Password)
		data.Data[i].AccountName, err = common.EncryptAES([]byte(data.Preferences.LockPwd), data.Data[i].AccountName)
	}
	if err != nil {
	...
	//  再用启动密码密钥加密启动密码
	data.Preferences.LockPwd, err = common.EncryptAES([]byte(common.AppStartPwdKeyAES), loadedItems.Preferences.LockPwd)
